...
const getRunWarningMessage = (run: CrawlRun) => {
  ...
  if (run.status === 'running') return `Crawl is still running.${run.error_message ? ` Error: ${run.error_message}.` : ''} Wait for completion before treating this source as healthy.`;
...
const SourceVerificationCard = ({ source }: { source: CrawlSourceStatus }) => {
  ...
  return (
    ...
    >
      ...
      <Text mt={2} color="whiteAlpha.500" fontSize="xs" fontFamily="mono" overflowWrap="anywhere">
        Public route: {source.public_route}
      </Text>
