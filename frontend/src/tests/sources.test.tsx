...
describe('SourcesPage', () => {
  ...
  test('renders all source cards with source boundaries, metadata, samples, and public links', () => {
    ...
    expect(
      screen.getByText((_, element) => element?.textContent === 'Public route: /holdings/dataroma'),
    ).toBeInTheDocument();
  ...
  test('renders a message when a successful status response is empty', () => {
    mockUseCrawlStatus.mockReturnValue(successfulResult([]));

    renderPage();

    expect(screen.getByRole('alert')).toHaveTextContent('No source status records returned');
  });
  ...
  test('includes the error message for a running crawl without dropping in-progress wording', () => {
    mockUseCrawlStatus.mockReturnValue(successfulResult([
      makeSource({
        latest_run: makeRun({
          status: 'running',
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('Crawl is still running');
    expect(screen.getByRole('alert')).toHaveTextContent('Worker heartbeat expired');
  });
